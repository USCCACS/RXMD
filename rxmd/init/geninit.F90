module params
implicit none
!integer,parameter :: vprocs(3)=(/6,6,6/)
!integer,parameter :: nio=36
!integer,parameter :: mc(3)=(/1,1,1/)

integer,parameter :: vprocs(3)=(/4,4,4/)
integer,parameter :: nio=2
...
integer :: natoms,  ntot, ix,iy,iz, ii
...
write(6,'(2a,1x,a,i6,3i3)') 'input file: ', trim(fname),' mctot,mc',mctot, mc(1:3)
...
print'(a,3x,6f8.3)','1, L2, L3, Lalpha, Lbeta, Lgamma: ', & 
...
!--- each atom has 32 (=3*8 + 8) bytes data
   ia=lnatoms1(sID)+lnatoms2(sID) 
   write(1,pos=ia*32+1) pos1(1:3,n),itype1(n)
