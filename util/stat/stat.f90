...
contains
  ...
  subroutine save_analysis_results(this) 
     class(analysis_context) :: this
     integer :: iunit,junit,ity,jty,k,kk
     real(8) :: dr, rho, dqk, Snq, Snq_denom, prefactor, prefactor2
     real(8),allocatable :: nr(:,:)
     ...
     open(newunit=junit,file='nr.dat',form='formatted')

     !print'(a $)', 'distance'
     write(unit=iunit,fmt='(a $)') ' distance'
     write(unit=junit,fmt='(a $)') ' distance'
     ...
     do jty=1,size(this%elems)
        ...
        write(unit=junit,fmt='(a5,a5 $)') & 
           ',    ',this%elems(ity)%str//'-'//this%elems(jty)%str
     enddo; enddo
     !print*
     write(unit=iunit,fmt=*)
     write(unit=junit,fmt=*)

     allocate(nr(size(this%elems),size(this%elems))); nr=0.d0; 
     do k=1,size(this%gr,dim=3)
        !print'(f10.5 $)', k/DRI
        dr = k/DRI
        prefactor = 4.d0*pi*dr*dr*rho/DRI
        write(unit=iunit,fmt='(f12.5 $)') dr
        write(unit=junit,fmt='(f12.5 $)') dr
        do ity=1,size(this%elems)
        do jty=1,size(this%elems)
          nr(ity,jty) = nr(ity,jty) + this%gr(ity,jty,k)
          write(junit, fmt='(f15.5,3x $)') nr(ity,jty)/this%num_sample_frames/this%num_atoms_per_type(ity)
          ...
          !nr(ity,jty) = nr(ity,jty) + this%gr(ity,jty,k)/DRI
          !write(junit, fmt='(es15.5,3x $)') nr(ity,jty)
        enddo; enddo
        !print*
        write(unit=iunit,fmt=*)
        write(unit=junit,fmt=*)
